...
(function() {
    ...
    // Interceptem endSession de game-core per restaurar el random
    // abans que copiarResultats generi el salt.
    const _waitForGameCore = setInterval(function() {
        if (typeof endSession !== 'function') return;
        clearInterval(_waitForGameCore);
        const _originalEndSession = endSession;
        // No podem reassignar `endSession` perquè és const/let a game-core
        // Però copiarResultats s'executa manualment (clic de l'alumne),
        // així que restaurem el random al DOMContentLoaded final.
    }, 50);

    // Restaurem quan el joc s'acaba (l'alumne veu la pantalla final)
    // Fem un observer que detecta quan #final-screen es fa visible
    window.addEventListener('DOMContentLoaded', function() {
