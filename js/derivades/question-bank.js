 ...
 * - FASE 9: Afegides dues famílies trigonomètriques:
 *     'chain-sin-int' → f(x) = sin(kx), f'(x) = k·cos(kx)
 *     'chain-cos-int' → f(x) = cos(kx), f'(x) = −k·sin(kx)
 *   k ∈ {−3,−2,−1,1,2,3} (enters, inclosos negatius per treballar el signe)
 *   URLs d'exemple:
 *     ?families=chain-sin-int
 *     ?families=chain-cos-int
 *     ?families=chain-sin-int,chain-cos-int
 *     ?families=chain-exp-int,chain-sin-int,chain-cos-int,power
...
/** Retorna l'argument kx en LaTeX: '3x', '-2x', 'x', '-x' */
function _kxArg(k) {
    if (k ===  1) return 'x';
    if (k === -1) return '-x';
    return `${k}x`;
...
/** Formata k·fn(kx) en LaTeX: gestiona k=±1 */
...
/** Afegeix parèntesis si l'expressió conté + o - a nivell arrel (fora de {}) */
...
// =========================================================================
// FAMÍLIA: sin(kx) — derivada k·cos(kx)
// =========================================================================
function generateSinKxInt() {
    // k ∈ {−3,−2,−1,1,2,3}: inclou negatius per treballar el signe de la cadena
    // k=1 i k=-1 amb doble pes (cas base, sense coeficient visible)
    ...
    const fallbacks = [
        { tex: `\\cos(${arg})`,          feedback: "Has derivat sin a cos, però has oblidat multiplicar per la derivada de l'argument (k).", errorType: 'CHAIN_FORGOT',  scope: 'family:sin' },
        { tex: _trigTerm(k, 'sin', arg), feedback: "La derivada de sin és cos, no sin. Sin i cos s'intercanvien.",                           errorType: 'SIN_COS_SWAP', scope: 'family:sin' },
    ...
    return {
        ...
        solutionTex,
        options: [
            { tex: solutionTex, feedback: "Molt bé! Recorda: (sin(kx))' = k·cos(kx).", errorType: null, isCorrect: true },
            ...distractors.map(d => ({ tex: d.tex, feedback: d.feedback, errorType: d.errorType, isCorrect: false }))
        ],
        meta: { family: 'chain-rule', outerFn: 'sin', innerFn: 'linear-int', params: { k }, ruleLabel: 'Regla de la cadena' }
    };
}

// =========================================================================
// FAMÍLIA: cos(kx) — derivada −k·sin(kx)
// =========================================================================
function generateCosKxInt() {
    ...
    const fallbacks = [
        { tex: _trigTerm(k, 'sin', arg),  feedback: "Has derivat cos a sin, però has oblidat el signe negatiu: (cos u)' = −sin(u)·u'.", errorType: 'CHAIN_SIGN',   scope: 'family:cos' },
        { tex: _trigTerm(-k, 'cos', arg), feedback: "Has posat el signe negatiu, però la derivada de cos és −sin, no −cos.",            errorType: 'SIN_COS_SWAP', scope: 'family:cos' },
    ...
    return {
        ...
        solutionTex,
        options: [
            { tex: solutionTex, feedback: "Molt bé! Recorda: (cos(kx))' = −k·sin(kx).", errorType: null, isCorrect: true },
            ...distractors.map(d => ({ tex: d.tex, feedback: d.feedback, errorType: d.errorType, isCorrect: false }))
        ],
        meta: { family: 'chain-rule', outerFn: 'cos', innerFn: 'linear-int', params: { k }, ruleLabel: 'Regla de la cadena' }
    };
}

// =========================================================================
// CATÀLEG DE PARELLS (producte i quocient) — sense canvis
// =========================================================================
...
function generateProduct() {
    ...
    const distractors = _selectDistractors(pool, solutionTex, 3, fallbacks);
    return {
        promptTex: `f(x) = ${promptTex}`, solutionTex,
        options: [{ tex: solutionTex, feedback: "Molt bé! Has aplicat correctament la regla del producte: (fg)' = f'g + fg'.", errorType: null, isCorrect: true }, ...distractors.map(d => ({ tex: d.tex, feedback: d.feedback, errorType: d.errorType, isCorrect: false }))],
...
function generateQuotient() {
    ...
    return {
        promptTex: `f(x) = ${promptTex}`, solutionTex,
        options: [{ tex: solutionTex, feedback: "Molt bé! Has aplicat correctament la regla del quocient: (f'g − fg')/g².", errorType: null, isCorrect: true }, ...distractors.map(d => ({ tex: d.tex, feedback: d.feedback, errorType: d.errorType, isCorrect: false }))],
