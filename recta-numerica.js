...
(function() {
    ...
    // ── 4a. SAFETY NET: beforeunload ────────────────────────────────────
    //    [FIX C2] Si l'alumne abandona la sessió (recàrrega, clic a Inici,
    //    tancament de pestanya) ABANS d'arribar a la pantalla final,
    //    restaurem Math.random per evitar que quedi determinista en cas de
    //    navegació dins la mateixa sessió (ex: back/forward cache, BFCache).
    window.addEventListener('beforeunload', function() {
        Math.random = _originalRandom;
    });

    // ── 4b. Restaurem quan el joc s'acaba (pantalla final visible) ──────
    //    El MutationObserver detecta quan #final-screen es fa visible.
    window.addEventListener('DOMContentLoaded', function() {
